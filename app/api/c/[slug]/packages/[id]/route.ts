...
export async function POST(req: NextRequest, { params }: Context) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: "Package not found." }, { status: 404 })
    ...
    if (!row || !canSee(row,auth.user.id,manageAll)) { await client.query("ROLLBACK"); return NextResponse.json({ error: "Package not found." }, { status: 404 }) }
    ...
    if (action === "open") {
      ...
      if (row.status === "Completed") { await client.query("ROLLBACK"); return NextResponse.json({ error: "Completed packages cannot be opened." }, { status: 409 }) }
      const ownerUserId = row.owner_user_id == null ? null : Number(row.owner_user_id)
      if (ownerUserId && ownerUserId !== auth.user.id && !manageAll) { await client.query("ROLLBACK"); return NextResponse.json({ error: "This package is assigned to another member." }, { status: 409 }) }
    ...
    if (action === "assign") {
      if (!manageAll) { await client.query("ROLLBACK"); return NextResponse.json({ error: "Package not found." }, { status: 404 }) }
      ...
      const ownerUserId = row.owner_user_id == null ? null : Number(row.owner_user_id)
      if (!manageAll && ownerUserId !== auth.user.id && Number(row.uploaded_by_user_id) !== auth.user.id) { await client.query("ROLLBACK"); return NextResponse.json({ error: "Package not found." }, { status: 404 }) }
      ...
      await client.query("ROLLBACK"); return NextResponse.json({ error: "Unknown package action." }, { status: 400 })
    }
    const packageData = await currentPackage(client,auth.congregation.id,id,auth.user.id,manageAll)
    await client.query("COMMIT")
...
export async function PATCH(req: NextRequest, { params }: Context) {
  ...
  try {
    ...
    if(body?.name!==undefined){const name=validatePackageName(body.name);if(!name)return NextResponse.json({error:"Package name is required."},{status:400});fields.push(`name=$${values.push(name)}`)}
    ...
    if(!row||(!manageAll&&Number(row.uploaded_by_user_id)!==auth.user.id)){await client.query("ROLLBACK");return NextResponse.json({error:"Package not found."},{status:404})}
