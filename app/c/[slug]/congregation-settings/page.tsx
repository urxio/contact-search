...
export default async function CongregationSettingsPage({ params }: { params: { slug: string } }) {
  ...
  try {
    ...
    if (error instanceof AuthError && error.status === 401) {
      redirect(`/auth/sign-in?next=/c/${encodeURIComponent(params.slug)}/congregation-settings`)
    }
    notFound()
  }
  ...
  return (
    <PageFrame
      ...
      description="Manage members, invitations, territory coverage, and congregation details."
    >
      <SettingsWorkspace
        slug={params.slug}
        initialName={access.congregation.name || titleFromSlug(params.slug)}
      />
    </PageFrame>
  )
}
